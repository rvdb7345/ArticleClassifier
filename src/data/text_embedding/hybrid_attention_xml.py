...
abstracts_df = processed_df.loc[processed_df.pui.isin(label_columns.pui), ['abstract']].reset_index(drop=True)

train_indices, test_indices = train_test_split(range(len(label_columns)), test_size=0.2, random_state=0)

abstracts_df = abstracts_df['abstract'].apply(lambda x: [word_idx_dict[word] for word in x.split(' ')])

X_trn = embedding_df.loc[train_indices, embedding_df.columns.difference(['pui'])].to_numpy()
X_tst = embedding_df.loc[test_indices, embedding_df.columns.difference(['pui'])].to_numpy()

Y_trn = label_columns.loc[train_indices, label_columns.columns.difference(['pui'])].to_numpy()
Y_tst = label_columns.loc[test_indices, label_columns.columns.difference(['pui'])].to_numpy()
...
model = Hybrid_XML(num_labels=52, vocab_size=len(word_idx_dict), embedding_size=128, embedding_weights=embedding_weights,
                   max_seq=500, hidden_size=52, d_a=52, label_emb=label_emb)
